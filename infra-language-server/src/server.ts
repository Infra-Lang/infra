...
const defaultSettings: InfraSettings = { maxNumberOfProblems: 1000 };
let globalSettings: InfraSettings = defaultSettings;

// Cache the settings of all open documents
const documentSettings: Map<string, Promise<InfraSettings>> = new Map();

connection.onDidChangeConfiguration((change: DidChangeConfigurationParams) => {
  if (hasConfigurationCapability) {
    documentSettings.clear();
  } else {
    globalSettings = <InfraSettings>(
      (change.settings.infraLanguageServer || defaultSettings)
    );
  }
...
function getDocumentSettings(resource: string): Promise<InfraSettings> {
  if (!hasConfigurationCapability) {
        ...
        for (let i = 0; i < lines.length; i++) {
          ...
          if (functionMatch) {
            return Location.create(
              ...
              Range.create(i, functionMatch.index!, i, functionMatch.index! + functionMatch[0].length)
            );
          }
        }
      }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (functionMatch) {
        ...
        const selectionRange = Range.create(
          i, 
          functionMatch.index! + functionMatch[0].indexOf(functionName),
          i,
          functionMatch.index! + functionMatch[0].indexOf(functionName) + functionName.length
        );
      ...
      if (variableMatch) {
        ...
        const selectionRange = Range.create(
          i,
          variableMatch.index! + variableMatch[0].indexOf(variableName),
          i,
          variableMatch.index! + variableMatch[0].indexOf(variableName) + variableName.length
        );
...
function getKeywordDocumentation(keyword: string): string | undefined {
