...
impl Interpreter {
        ...
        match stmt {
            ...
            Stmt::Let { name, type_annotation, value } => {
                ...
                if let Some(expected_type) = type_annotation {
                    self.check_type_compatibility_detailed(&val, expected_type, &format!("variable '{}'", name))?;
                }
                
                self.evaluator.define_variable(name.clone(), val);
                Ok(())
            }
            Stmt::Print(expr) => {
            ...
            Stmt::For { var, start, end, body } => {
                ...
                let (start_num, end_num) = match (start_val, end_val) {
                    (crate::core::Value::Number(s), crate::core::Value::Number(e)) => (s as i64, e as i64),
                    _ => return Err(crate::core::InfraError::TypeError {
                ...
                for i in start_num..end_num {
                    self.evaluator.define_variable(var.clone(), crate::core::Value::Number(i as f64));
                    self.execute_statement(body)?;
                }
            ...
            Stmt::Assignment { target, value } => {
                ...
                let temp_stmt = Stmt::Assignment { target: target.clone(), value: value.clone() };
                ...
                match self.execute_statement(try_block) {
                    ...
                    Err(error) => {
                        ...
                        if can_catch {
                            ...
                            self.evaluator.define_variable(catch_var.clone(), crate::core::Value::String(error_message));
            ...
            Stmt::Import { module_path, items, alias } => {
                ...
                let current_dir = self.current_file_path
                    .as_ref()
                    .and_then(|p| p.parent())
                ...
                match items {
                    ...
                    ImportItems::Named(import_items) => {
                        ...
                        for import_item in import_items {
                            if let Some(value) = module.exports.get(&import_item.name) {
                                ...
                                self.evaluator.define_variable(import_name.clone(), value.clone());
                            } else {
                                return Err(crate::core::InfraError::RuntimeError {
                                    message: format!("Export '{}' not found in module '{}'", import_item.name, module_path),
                                });
                            }
                        }
                    }
                    ImportItems::Default(name) => {
                        ...
                        if let Some(default_export) = module.exports.get("default") {
                            self.evaluator.define_variable(import_name.clone(), default_export.clone());
                        } else {
                            ...
                            self.evaluator.define_variable(import_name.clone(), crate::core::Value::Object(exports_obj));
                        }
                    }
                }
                Ok(())
    ...
    fn check_type_compatibility(&self, value: &Value, expected_type: &Type) -> bool {
        match (value, expected_type) {
            ...
            (Value::Array(arr), Type::Array(element_type)) => {
                ...
                arr.iter().all(|val| self.check_type_compatibility(val, element_type))
            }
            ...
            (value, Type::Union(types)) => {
                ...
                types.iter().any(|t| self.check_type_compatibility(value, t))
            }
    ...
    fn check_type_compatibility_detailed(&self, value: &Value, expected_type: &Type, context: &str) -> Result<()> {
        match (value, expected_type) {
                ...
                for (index, val) in arr.iter().enumerate() {
                    if !self.check_type_compatibility(val, element_type) {
                        return Err(crate::core::InfraError::TypeError {
                            expected: format!("{} to have array element at index {} of type {}", context, index, self.type_to_string(element_type)),
                    ...
                    match obj.get(prop_name) {
                        Some(val) => {
                            if !self.check_type_compatibility(val, prop_type) {
                                return Err(crate::core::InfraError::TypeError {
                                    expected: format!("{} to have property '{}' of type {}", context, prop_name, self.type_to_string(prop_type)),
                                    found: format!("{} ({})", val.type_name(), val)
                                });
                            }
                        }
                        None => {
                            return Err(crate::core::InfraError::TypeError {
                                expected: format!("{} to have required property '{}'", context, prop_name),
            ...
            _ => {
                Err(crate::core::InfraError::TypeError {
                    expected: format!("{} to be of type {}", context, self.type_to_string(expected_type)),
                    found: format!("{} ({})", value.type_name(), value)
                })
            }
        }
